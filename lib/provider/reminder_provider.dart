...
class ReminderProvider extends ChangeNotifier {
  ...
  TimeOfDay _reminderTime = const TimeOfDay(hour: 11, minute: 0); // Default 11:00
  ...
  Future<void> setReminderTime(TimeOfDay time) async {
    _reminderTime = time;
    NotificationService.setReminderTime(time); // Set time in notification service
    await _saveReminderSettings();
  ...
  Future<void> _saveReminderSettings() async {
    try {
      ...
      await prefs.setString(_reminderTimeKey, '${_reminderTime.hour}:${_reminderTime.minute}');
    } catch (e) {
  ...
  String get nextReminderText {
    ...
    final todayAtTime = DateTime(now.year, now.month, now.day, _reminderTime.hour, _reminderTime.minute);
